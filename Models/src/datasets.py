...
class BallsDataset(torch.utils.data.Dataset):
    def __init__(self, path="./data/interaction-10fps-4ballsize/train_gt.pkl", n_obj=3):
        ...
        for key in data_dict:
            if key != 'edges':
                ...
                if frame_id not in frames_id_list:
                    frames_id_list[frame_id] = {}
                    frames_id_list[frame_id]['ids'] = []
                    frames_id_list[frame_id]['ids'].append(key)
                    # each_frame_data = np.zeros((n_obj, 100, 3), dtype=np.float32)
                    # -5 to ensure the empty objects are not in the scene
                    # considering the (mean_x, std_x, mean_y, std_y) in normalized data
                    each_frame_data = np.full((n_obj, 100, 3), -5, dtype=np.float32)
                    each_frame_data[0,:,:] = np.asarray(data_dict[key], dtype=np.float32)
                    frames_id_list[frame_id]['data'] = each_frame_data
                    each_frame_sign = np.zeros((n_obj), dtype=np.float32)
                    each_frame_sign[0] = 1
                    frames_id_list[frame_id]['sign'] = each_frame_sign
                else:
                    frames_id_list[frame_id]['ids'].append(key)
                    each_frame_data = frames_id_list[frame_id]['data']
                    each_frame_data[len(frames_id_list[frame_id]['ids'])-1,:,:] = np.asarray(data_dict[key], dtype=np.float32)
                    frames_id_list[frame_id]['data'] = each_frame_data
                    each_frame_sign = frames_id_list[frame_id]['sign']
                    each_frame_sign[len(frames_id_list[frame_id]['ids'])-1] = 1
                    frames_id_list[frame_id]['sign'] = each_frame_sign
        ...
        self.sign_aggre = []
        self.edge_aggre = []
        for key in frames_id_list:
            each_frame_data = frames_id_list[key]['data']
            self.data_aggre.append(each_frame_data)
            each_frame_sign = frames_id_list[key]['sign']
            self.sign_aggre.append(each_frame_sign)
            each_frame_edge = self.edges[int(key)] # 1,0; 2,0; 2,1; 3,0; 3,1; 3,2
            self.edge_aggre.append(each_frame_edge)
        
    def __getitem__(self, i):
        return self.data_aggre[i], self.sign_aggre[i], self.edge_aggre[i] # shape: (n_obj,T,3) (n_obj) (T,n_edge_type)
