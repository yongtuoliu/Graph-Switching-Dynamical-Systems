...
from .forward_backward import forward_backward_hmm_no_interaction, forward_backward_hmm_interaction_simple
from .forward_backward import forward_backward_hsmm
...
class Base(nn.Module):
    def __init__(
        ...
        interaction_simple: bool = False,
        interaction_gnn: bool = False,
        interaction_gnn_fully_connected: bool = False,
        interaction_gnn_est_edge: bool = False,
        interaction_gnn_gt_edge: bool = False,
    ):
        """Base Model class.
    ...
    def _calculate_discrete_transition_probabilities(
        ...
        edge_est: torch.Tensor,
        edge_gt: torch.Tensor,
        ctrl_feats: Optional[torch.Tensor],
        temperature: float = 1.0,
    ):
        # print(y.shape, x.shape, hidden_states.shape) # B, O, T, D
        # print(edge_est.shape) # B, T, edge_number (with direct, O*(O-1)), edge_type 
        # print(edge_gt.shape) # B, T, edge_number (no direct, O*(O-1)//2)
        B, O, T, _ = y.shape
        ...
        if self.interaction_simple:
            log_prob_zt_ztm1 = log_prob_zt_ztm1.view([B, O, T-1, O*self.K, self.K])
        else:
            log_prob_zt_ztm1 = log_prob_zt_ztm1.view([B, O, T-1, self.K, self.K])
        ...
        if log_prob_zt_ztm1.is_cuda:
            if self.interaction_simple:
                identity_pad = torch.cuda.FloatTensor(B, O, 1, O*self.K, self.K).fill_(0.) + get_precision(y)
            else:
                identity_pad = torch.cuda.FloatTensor(B, O, 1, self.K, self.K).fill_(0.) + get_precision(y)
        else:
            if self.interaction_simple:
                identity_pad = torch.zeros(B, O, 1, O*self.K, self.K) + get_precision(y)
            else:
                identity_pad = torch.zeros(B, O, 1, self.K, self.K) + get_precision(y)
            
        if self.interaction_simple:
            identity_pad_uni = torch.eye(self.K, self.K, device=y.device).view(
                1, 1, 1, self.K, self.K).repeat(B, 1, 1, 1, 1) + get_precision(y)
            # identity_pad_uni = identity_pad_uni.log()
            for idx_obj in range(O):
                identity_pad[:,idx_obj:(idx_obj+1),:,idx_obj*self.K:(idx_obj+1)*self.K,:] = identity_pad_uni
            log_identity_pad = normalize_logprob(
                identity_pad, axis=-1, temperature=temperature
            )[0]
        else:
            identity_pad = torch.eye(self.K, self.K, device=y.device).view(
                1, 1, 1, self.K, self.K).repeat(B, O, 1, 1, 1) + get_precision(y)
            log_identity_pad = normalize_logprob(
                identity_pad, axis=-1, temperature=temperature
            )[0]
        #  Pad the [:, 0, :, :] elements with log I. This is to ensure
        #  correct dimensions and the 0-th index is not used in computations.
        log_prob_zt_ztm1 = torch.cat([log_identity_pad, log_prob_zt_ztm1], dim=2)
    ...
    def _calculate_likelihoods(
        ...
        # print(y.shape, x_samples.shape)
        B_old, O, T, _ = y.shape
        y = y.view(B_old*O, T, -1)
        x_samples = x_samples.view(B_old*O, T, -1)
        ...
        log_p_xt_yt = log_p_xt_yt.view(B_old, O, T, -1)
        return log_p_xt_yt
...
class SNLDS(Base):
    def __init__(
        ...
        super().__init__(
            ...
            interaction_simple=interaction_simple,
            interaction_gnn=interaction_gnn,
            interaction_gnn_fully_connected=interaction_gnn_fully_connected,
            interaction_gnn_est_edge=interaction_gnn_est_edge,
            interaction_gnn_gt_edge=interaction_gnn_gt_edge,
        )
    ...
    def _get_iwlbo(
        ...
        # iwlbo = iwlbo.mean(0)
        iwlbo = torch.sum(iwlbo*y_sign.view(-1)) / torch.sum(y_sign)
        return iwlbo

    def _calculate_objective_no_simple_interaction(
        self,
        y_sign: torch.Tensor,
        log_a: torch.Tensor,
        log_b: torch.Tensor,
        ...
        entropy_term = torch.sum(torch.sum(x_entropy, dim=2)*y_sign) / torch.sum(y_sign)
    ...
    def _calculate_objective_interaction_simple(
        self,
        y_sign: torch.Tensor,
        log_a: torch.Tensor,
        log_b: torch.Tensor,
        log_z_init: torch.Tensor,
        bwd_for_consistency_loss: torch.Tensor,
        log_gamma: torch.Tensor,
        log_xi: torch.Tensor,
        log_prob_q: torch.Tensor,
        x_entropy: torch.Tensor,
        ...
        # for consistency loss in backward algorithm
        B, O, T, OK = bwd_for_consistency_loss.shape
        elbo_consistency_loss = []
        for idx_i in range(O):
            for idx_j in range(idx_i):
                elbo_consistency_loss.append(torch.abs(bwd_for_consistency_loss[:,idx_i,:,:] - bwd_for_consistency_loss[:,idx_j,:,:]).mean())
        elbo_consistency_loss = sum(elbo_consistency_loss)/len(elbo_consistency_loss)
    ...
    def _calculate_discrete_transition_and_forward_backward_for_gnn_interaction(
        self,
        y: torch.Tensor,
        x: torch.Tensor,
        hidden_states: torch.Tensor,
        ...
        B, O, T, K = log_b.shape
        device = log_b.device
        log_a = torch.zeros(B, O, T, K, K, device=device)
        a_0 = torch.eye(K, K, device=device).view(
                1, 1, 1, self.K, self.K).repeat(B, O, 1, 1, 1) + get_precision(y)
        log_a_0 = normalize_logprob(a_0, axis=-1, temperature=temperature)[0]
        log_a[:,:,0:1,:,:] = log_a_0

        # forward algorithm and calculate log_a
        forward_probs = torch.zeros(B, O, T, K, device=device)
        normalizers = torch.zeros(B, O, T, device=device)
        forward_probs[:, :, 0, :], normalizers[:, :, 0] = normalize_logprob(
            log_z_init[:, :, :] + log_b[:, :, 0, :], axis=-1
        )
        for t in range(1, T):
            log_a_t = self.z_tran(
                y[:, :, t-1:t, :],
                x[:, :, t-1:t, :],
                hidden_states[:, :, t-1:t, :],
                torch.exp(forward_probs[:, :, t-1:t, :]),
                edge_est[:, t-1:t, :, :],
                edge_gt[:, t-1:t, :],
                ctrl_feats[:, t:t+1, :] if ctrl_feats is not None else None,
            ).view([B, O, K, K])
            log_a_t = normalize_logprob(
                log_a_t, axis=-1, temperature=temperature
            )[0]
            log_a[:,:,t,:,:] = log_a_t
            _ones = torch.ones(B, O, K, device=device)
            _A_summed = torch.exp(log_a_t).sum(-1)
            ...
            log_a_t = torch.transpose(log_a_t, -2, -1)
            log_b_t = log_b[:, :, t, :]
            prev_prob = forward_probs[:, :, t-1, :]
            alpha = torch.logsumexp(
                log_b_t[:, :, :, None] + log_a_t + prev_prob[:, :, None, :], dim=-1
            )
            forward_probs[:, :, t, :], normalizers[:, :, t] = normalize_logprob(alpha, axis=-1)

        # backward algorithm
        backward_probs = torch.zeros(B, O, T, K, device=device)
        ...
        log_xi_flat, _ = normalize_logprob(log_xi.reshape(B, O, T-1, K * K), axis=-1)
        log_xi = log_xi_flat.view_as(log_xi)
        # padding the matrix to the same shape of inputs
        log_uniform_pad = torch.log(torch.ones([B, O, 1, K, K], device=device) / (K * K))
        log_xi = torch.cat([log_uniform_pad, log_xi], dim=2)
        log_p_XY = fwd_normalizers.sum(-1)

        return fwd, bwd, log_gamma, log_xi, log_p_XY, log_a
    ...
    def forward(
        ...
        # print(y.shape, y_sign.shape, y_edge.shape)
        ...
        x_samples, x_entropy, log_prob_q, hidden_states, edge_est = self.inference_network(
            y,
        ...
        _, B, O, T, x_dim = x_samples.shape
        #  Merge the first two dimensions into a single batch dimension
        x_samples = x_samples.view(num_samples * B, O, T, x_dim)
        x_entropy = x_entropy.view(num_samples * B, O, T)
        log_prob_q = log_prob_q.view(num_samples * B, O, T)
        hidden_states = hidden_states.view(num_samples * B, O, T, -1)

        #  Repeat the first dim num_samples times to allow broadcast with x_samples.
        y_tiled = y.repeat(num_samples, 1, 1, 1)
        y_sign_tiled = y_sign.repeat(num_samples, 1)
        ...
        if ctrl_feats_tiled is None:
            ...
            dummy_ctrls = torch.ones(y_tiled.size(0), O, 1, device=y.device)
        ...
        log_b = self._calculate_likelihoods(
            y_tiled,
            x_samples,
            target_transformer.scale.repeat(num_samples, 1, 1, 1),
            ctrl_feats_tiled,
        )

        if self.no_interaction:
            #  Compute log A, A = p(z[t] | x[t-1], z[t-1]).
            edge_gt = y_edge.repeat(num_samples, 1, 1)
            log_a = self._calculate_discrete_transition_probabilities(
                y_tiled, x_samples, hidden_states, edge_est, edge_gt, ctrl_feats_tiled, temperature=switch_temperature
            ) # log_a.shape (B, O, T, N_cat, N_cat)
            #  Compute gamma, xi, and, log p(X, Y) using the forward-backward algorithm.
            _, _, log_gamma, log_xi, log_p_XY = forward_backward_hmm_no_interaction(
                log_a, log_b, log_z_init,
            )
            #  Compute the objective function.
            elbo, elbov2, iwlbo, iwlbov2 = self._calculate_objective_no_simple_interaction(
                y_sign_tiled,
                log_a,
                log_b,
                log_z_init,
            ...
            log_a = self._calculate_discrete_transition_probabilities(
                y_tiled, x_samples, hidden_states, edge_est, edge_gt, ctrl_feats_tiled, temperature=switch_temperature
            ) # log_a.shape (B, O, T, O*N_cat, N_cat)
            #  Compute gamma, xi, and, log p(X, Y) using the forward-backward algorithm.
            _, _, bwd_for_consistency_loss, log_gamma, log_xi, log_p_XY = forward_backward_hmm_interaction_simple(
                log_a, log_b, log_z_init,
            )
            #  Compute the objective function.
            elbo, elbov2, iwlbo, iwlbov2 = self._calculate_objective_interaction_simple(
                y_sign_tiled,
                log_a,
                log_b,
                log_z_init,
                bwd_for_consistency_loss,
                log_gamma,
                log_xi,
                log_prob_q,
                cont_ent_anneal * x_entropy,
            ...
            edge_gt = y_edge.repeat(num_samples, 1, 1)
            _, _, log_gamma, log_xi, log_p_XY, log_a= self._calculate_discrete_transition_and_forward_backward_for_gnn_interaction(
                y_tiled, 
                x_samples, 
                hidden_states, 
                log_b, 
        ...
        recons_y = self.get_reconstruction(x_samples).view([num_samples, B, O, T, -1])
        ...
        # )
        crossent_regularizer = (
            torch.sum(torch.einsum("ijkz, z -> ijk", log_gamma, self.discrete_prior).sum(2)*y_sign_tiled) / torch.sum(y_sign_tiled)
        )
        log_gamma = log_gamma.view([num_samples, B, O, T, -1])
        x_samples = x_samples.view([num_samples, B, O, T, x_dim])
...
class REDSDS(Base):
    ...
    def forward(
        ...
        log_a = self._calculate_discrete_transition_probabilities(
            y_tiled, x_samples, ctrl_feats_tiled, temperature=switch_temperature
        )
        #  Get the NSTFs
        if ctrl_feats_tiled is None:
