...
def evaluate_forecast(true_futures, forecasts, freq="H"):
    ...
    idx = pd.date_range(start=dummy_time_stamp, periods=prediction_len, freq=freq)
    if obs_dim == 1:
...
def segmentation_accuracy(true_seg, predicted_seg):
    ...
    mapper = {unique_preds[c]: unique_labels[r] for (r, c) in zip(row_idx, col_idx)}
...
def evaluate_segmentation(true_seg, predicted_seg, K):
    ...
    metrics = dict(nmi_score=nmi_score, ari_score=ari_score, accuracy=acc, f1_score=f1)
    return metrics
